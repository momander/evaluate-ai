...
def print_accuracy_results(
    ...
    for model_name in model_names:
        ...
        if total > 0:
            ...
            print(
                f"Model \"{model_name}\": No evaluations for this model."
            )

def print_explanation_similarity_results(
    model_names: list, explanation_similarities: dict
):
    ...
    for model_name in model_names:
        if explanation_similarities[model_name]:
            avg_similarity = (
                sum(explanation_similarities[model_name])
                / len(explanation_similarities[model_name])
            )
            print(
...
# Define a single prompt template
prompt_template = (
...
# Loop through messages and models for evaluation
